                                    ...
                                    <div class="card-body">
                                        <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php 
                                                    if($manufacturer['featured'] == 1 )
                                                    {
                                                        echo '<input class="feature" type="checkbox" id="" value="'.$manufacturer['id'].'" checked>';
                                                    }else{
                                                        echo '<input class="feature" type="checkbox" id="" value="'.$manufacturer['id'].'"';
                                                    } 
                                                    ?>
                                                </td>
                                                ...
                                                <td><a href="#" data-toggle="modal" data-target=".bs-example-modal-lgedit" onclick="get_product_edit(<?php echo $manufacturer['id']; ?>)"><i class="fas fa-edit"></i></a> | 
                                                    <a href="<?php echo base_url('admin/manufacturer/delete/'.$manufacturer['id']) ?>" onclick="return confirm('Are you sure you want to remove the contact list?')" data-toggle="tooltip" data-original-title="Delete" class="far fa-trash-alt"></a></td>
        ...
        <div class="modal-body">
                ...
                <div class="modal-body">
                 ...
                 <?php $this->load->helper('form'); ?>
                    ...
                    <?php echo form_open(base_url() . 'admin/manufacturer/add' , array('class' => 'form-horizontal form-groups-bordered validate', 'enctype' => 'multipart/form-data')); ?>

                                            <div class="form-group row">
                                                <label class="col-sm-3 col-form-label">Manufaturer</label>
                                                <div class="col-sm-9">
    ...
    <div class="modal-content" id="ajax-content-load">
        ...
        <script type="text/javascript">
            function get_product_edit(id) {
                $.ajax({
                    url: '<?php echo base_url();?>admin/manufacturer/update/' + id ,
            ...
            $(document).ready( function () {
                ...
                table.$('.feature').on('change', function(){
                    var id = $(this).val();
                    if($(this).is(":checked")) {
                        var status = 1;
                        var message = 'Manufacturer Set As Featured Successfully!';
                    }else{
                        var status = 0;
                        var message = 'Manufacturer Removed As Featured Successfully!';
                    }
                    var post_data = {
                        'id': id,
                        'status': status,
                    ...
                    $.ajax({
                        url : '<?= base_url(); ?>admin/manufacturer/updateFeature',
                        type : 'POST',
                        data : post_data,
