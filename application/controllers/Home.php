...
class Home extends CI_Controller
{
    ...
    public function product($name = "")
    {
        ...
        if (!empty($name) && !is_numeric($name)) 
        {
            ...
            $config["total_rows"] = $total_rows = $total_rows = $this->common_model->get_all_product_by_manufacture_rows($newName);
        }
        else{
            $config["uri_segment"] = 3;
            $config["base_url"] = base_url(). "home/product/";
            $config["total_rows"] = $total_rows = $total_rows = $this->common_model->get_all_product_rows();            
        }
        ...
        if (!empty($name)) 
        {
            ...
            // echo $newName;exit;
            $data['product'] = $this->common_model->get_all_product_by_mnfr($config["per_page"], $page, $newName);
        } 
        else
        {
            $page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
            $data['product'] = $this->common_model->get_all_products($config["per_page"], $page);
        }
